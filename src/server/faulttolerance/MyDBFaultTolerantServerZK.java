...
public class MyDBFaultTolerantServerZK extends server.MyDBSingleServer implements Watcher, StatCallback{
	...
	public void process(WatchedEvent event) {
        ...
        if (event.getType() == Event.EventType.None) {
			...
			} else if (path != null && path.equals("/server0")) {
				//Some change has happened on /server0. It can either be created or deleted, nothing else
				if (event.getType() == Event.EventType.NodeCreated) {

				} else if (event.getType() == Event.EventType.NodeDeleted) {

				}
			} else if (path != null && path.equals("/server1")) {

			} else if (path != null && path.equals("/server2")) {

			} else {
				System.out.println("Notification of some other znode. NOt interested!!!!");
			}
        }
	...
	public void createServerZnode(String servername) {
		try {
			// Check if the znode already exists
			Stat stat = zooKeeper.exists("/" + servername, event -> process(event));
			if (stat == null) {
				// Znode leader does not exist, create it
				String firstZnodeName = zooKeeper.create("/" + servername, servername.getBytes(),
						ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
				System.out.println("SErver Znode created: " + firstZnodeName);
				return;
			} 
	...
	private String getLeaderZnodes(String servername) {
		try {
			// Check if the znode already exists
			Stat stat = zooKeeper.exists("/leader", event -> process(event));
			if (stat == null) {
				// Znode leader does not exist, create it
	...
	private boolean dequeue(String node) {
        ...
        // try (BufferedReader reader = new BufferedReader(new FileReader("conf/servers.properties"))) {
        //     properties.load(reader);
        // } catch (IOException e) {
        //     e.printStackTrace();
        // }
		// for (String serverKey : properties.stringPropertyNames()) {
		// 	//StringBuilder output = new StringBuilder();
        //     String serverAddress = properties.getProperty(serverKey);
		// 	if(isZooKeeperServerDown(serverAddress)){
		// 		System.out.print(serverKey);
		// 		String [] downserver = serverKey.split("r.");
		// 		serverslist.remove(downserver[1]);
	...
	// private boolean isZooKeeperServerDown(String serverAddress) {
	// 	try (ZooKeeper zk = new ZooKeeper(serverAddress, 1000, null)) {
	// 		Stat stat = zk.exists("/", false);
	// 		return stat == null; // If stat is null, the server is considered down
	// 	} catch (IOException | InterruptedException | KeeperException e) {
	// 		return true; // Consider the server down in case of exceptions
	// 	}
	// }
